...
on_update(function ()
    ...
    if target_selector_data.has_elite then
        ...
        local distance_sqr = unit_position:squared_dist_to_ignore_z(cursor_position);
    ...
    local distance_sqr = best_target_position:squared_dist_to_ignore_z(cursor_position);

    if distance_sqr > (max_range * max_range) then
        best_target = target_selector_data.closest_unit;
        local closer_pos = best_target:get_position();
        local distance_sqr_2 = closer_pos:squared_dist_to_ignore_z(cursor_position);
